...
def airfoil_validation():
    ...
    ax6.set_xlabel('Angle of attack, deg')
    ax6.set_ylabel('Lift coefficient')
    ax6.hold(True)
    ax6.plot(exp.alpha,exp.cl,'ro')
    ax6.plot(cfd1.alpha,cfd1.cl,'k-')
    ax6.legend(['experiment','k-epsilon'],'lower right')
    ax6.axis([-10,15,None,None])
    
    #fig7 = plt.figure(7)
    ax7 = fig6.add_subplot(212)
    ax7.set_xlabel('Angle of attack, deg')
    ax7.set_ylabel('Drag coefficient')
    ax7.hold(True)
    ax7.plot(exp.alpha,exp.cd,'ro')
    ax7.plot(xfoil.alpha,xfoil.cd,'k-')
    ax7.legend(['experiment','xfoil'],'upper left')
    ax7.axis([-10,15,0.0,0.1])
